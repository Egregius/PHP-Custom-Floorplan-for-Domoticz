...
function Schakel($idx,$cmd,$name=NULL){
	global $user;
	lg('=> '.$user.' => switched '.$idx.' '.$name.' '.$cmd);
	file_get_contents('http://127.0.0.1:8084/json.htm?type=command&param=switchlight&idx='.$idx.'&switchcmd='.$cmd.'&level=0&passcode=');
}
function Scene($idx,$name=NULL){
	global $user;
	lg('=> '.$user.' =>	Scene	'.$name.'	'.$cmd.'	'.$reply);
	file_get_contents('http://127.0.0.1:8084/json.htm?type=command&param=switchscene&idx='.$idx.'&switchcmd=On');
}
function Dim($idx,$level,$name=NULL){
	global $user;
	lg('=> '.$user.' =>	Dimmer '.$idx.'	'.$name.'	'.$level);
...
function Udevice($idx,$nvalue,$svalue,$name=NULL){
	global $user;
	lg('=> '.$user.' =>	UDevice '.$idx.'	'.$nvalue.'	'.$svalue);
...
function Motion($name){
	global $Sweg,$Sslapen;
	echo($Sweg=='On'||$Sslapen=='On')
		?'<div class="fix motionr '.$name.'"></div>'
		:'<div class="fix motion '.$name.'"></div>';}
